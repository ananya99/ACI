...
class CustomCallbacks(DefaultCallbacks):
    """Custom callbacks for RLlib training."""

    def on_episode_start(self, *, worker, base_env, policies, episode: Dict, **kwargs):
        """Called at the start of each episode."""
        print(f"Episode {episode['episode_id']} started.")
        episode["user_data"] = {"rewards": []}

    def on_episode_step(self, *, worker, base_env, episode: Dict, **kwargs):
        """Called at each step of an episode."""
        # Example: Track rewards
        reward = episode.get("last_reward_for", None)
        if reward is not None:
            episode["user_data"]["rewards"].append(reward)

    def on_episode_end(self, *, worker, base_env, policies, episode: Dict, **kwargs):
        """Called at the end of each episode."""
        total_reward = sum(episode["user_data"]["rewards"])
        print(f"Episode {episode['episode_id']} ended with total reward: {total_reward}")
...
class MjCambrianModel:
    """Custom model wrapper for RLlib."""

    def __init__(self, env_creator: Callable, policy_kwargs: Optional[Dict] = None):
        """Initialize the model with an RLlib Trainer."""
        ...
        if self.policy_kwargs:
            self.config.training(model=self.policy_kwargs)
        self.trainer = self.config.build()
        self._rollout: Optional[List[Dict[str, Any]]] = None
    ...
    def predict(self, *args, **kwargs):
        ...
        if self._rollout is not None:
            return self._rollout.pop(0), None
        raise NotImplementedError("Prediction logic should be implemented based on RLlib.")